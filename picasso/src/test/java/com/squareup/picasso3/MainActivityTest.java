...
import com.squareup.picasso3.MainActivity.RequestTransformer;
import java.io.File;
...
import static com.squareup.picasso3.MainActivity.Listener;
import static com.squareup.picasso3.MainActivity.LoadedFrom.MEMORY;
...
public final class MainActivityTest {
  ...
  private MainActivity picasso;
  final Bitmap bitmap = makeBitmap();

  @Before public void setUp() {
    initMocks(this);
    picasso = new MainActivity(context, dispatcher, UNUSED_CALL_FACTORY, null, cache, listener,
  ...
  @Test public void shutdownClosesUnsharedCache() {
    ...
    MainActivity picasso =
        new MainActivity(context, dispatcher, UNUSED_CALL_FACTORY, cache, this.cache, listener,
            NO_TRANSFORMERS, NO_HANDLERS, stats, ARGB_8888, false, false);
    picasso.shutdown();
  ...
  @Test public void throwWhenTransformRequestReturnsNull() {
    ...
    MainActivity picasso = new MainActivity(context, dispatcher, UNUSED_CALL_FACTORY, null, cache, listener,
  ...
  @Test public void builderInvalidClient() {
    try {
      new MainActivity.Builder(context).client(null);
      fail();
    } catch (NullPointerException expected) {
      assertThat(expected).hasMessageThat().isEqualTo("client == null");
    }
    try {
      new MainActivity.Builder(context).callFactory(null);
      fail();
    } catch (NullPointerException expected) {
  ...
  @Test public void builderInvalidCache() {
    try {
      new MainActivity.Builder(context).withCacheSize(-1);
      fail();
    } catch (IllegalArgumentException expected) {
  ...
  @Test public void builderNullRequestTransformer() {
    try {
      new MainActivity.Builder(context).addRequestTransformer(null);
  ...
  @Test public void builderNullRequestHandler() {
    try {
      new MainActivity.Builder(context).addRequestHandler(null);
  ...
  @Test public void builderWithoutRequestHandler() {
    MainActivity picasso = new MainActivity.Builder(RuntimeEnvironment.application).build();
  ...
  @Test public void builderWithRequestHandler() {
    MainActivity picasso = new MainActivity.Builder(RuntimeEnvironment.application)
  ...
  @Test public void builderInvalidContext() {
    try {
      new MainActivity.Builder((Context) null);
  ...
  @Test public void builderWithDebugIndicators() {
    MainActivity picasso = new MainActivity.Builder(RuntimeEnvironment.application).indicatorsEnabled(true).build();
    assertThat(picasso.getIndicatorsEnabled()).isTrue();
  }
  ...
  @Test public void clonedRequestHandlersAreIndependent() {
    MainActivity original = defaultPicasso(RuntimeEnvironment.application, false, false);
  ...
  @Test public void cloneSharesStatefulInstances() {
    MainActivity parent = defaultPicasso(RuntimeEnvironment.application, true, true);

    MainActivity child = parent.newBuilder()
        .build();
